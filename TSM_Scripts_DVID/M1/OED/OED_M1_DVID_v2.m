function [out]=OED_M1_DVID_v2(epccOutputResultFileNameBase,epccNumLoops,stepd,epcc_exps,global_theta_guess)
...
%     inputs.plotd.plotlevel='min';
    ...
    clear newExps;
    newExps.n_exp = 1;
    newExps.n_obs{1}=2;                                      % Number of observables in the experiment
    newExps.obs_names{1}=char('RFP','GFP');               % Name of the observables in the experiment
    newExps.obs{1}=char('RFP = L_RFP','GFP = T_GFP');          % Observation function
    newExps.exp_y0{1}= [y0];
    ...
    % OPTIMIZATION
    inputs.nlpsol.nlpsolver='eSS';
    inputs.nlpsol.eSS.maxeval = 200000;%200000;
    inputs.nlpsol.eSS.maxtime = 5000;
    inputs.nlpsol.eSS.local.solver = 'fmincon';  
    inputs.nlpsol.eSS.local.finish = 'fmincon';  
